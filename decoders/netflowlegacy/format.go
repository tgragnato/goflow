...
func (p PacketNetFlowV5) String() string {
	...
	str.WriteString(fmt.Sprintf("  FlowSequence: %v\n", p.FlowSequence))
	str.WriteString(fmt.Sprintf("  EngineType: %v\n", p.EngineType))
	str.WriteString(fmt.Sprintf("  EngineId: %v\n", p.EngineId))
	str.WriteString(fmt.Sprintf("  SamplingInterval: %v\n", p.SamplingInterval))
	str.WriteString(fmt.Sprintf("  Records (%v):\n", len(p.Records)))
