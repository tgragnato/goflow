...
type TemplateStore interface {
	AddTemplate(ctx FlowContext, version uint16, obsDomainId uint32, templateId uint16, template any) (TemplateStatus, error)
	GetTemplate(ctx FlowContext, version uint16, obsDomainId uint32, templateId uint16) (any, error)
}

// ManagedTemplateStore adds lifecycle and operational methods on top of TemplateStore.
type ManagedTemplateStore interface {
	TemplateStore
	RemoveTemplate(ctx FlowContext, version uint16, obsDomainId uint32, templateId uint16) (any, bool, error)
